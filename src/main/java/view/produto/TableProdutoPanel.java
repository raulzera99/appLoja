...
public class TableProdutoPanel extends JPanel {
	private static final long serialVersionUID = -4694190107545197497L;
	JTable tableProduto;
	JScrollPane scrollPane = new JScrollPane();
	JScrollPane scrollPaneTableProduto = new JScrollPane();
	...
	private TableProdutoModel model;
	private Page<Produto> page;
	private ProdutoService produtoeService;
	...
	private static TableProdutoPanel TABLE_CLIENTE;

	public TableProdutoPanel() {
		//setVisible(true);
		initComponents();
		eventHandler();
		initTable();
	...
	private void eventHandler() {
		...
		btnAdicionar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				showProdutoFrame(Constantes.INCLUIR);
				initTable();
			}
		...
		btnAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showProdutoFrame(Constantes.ALTERAR);
				initTable();
			}
		});
		btnRemover.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showProdutoFrame(Constantes.EXCLUIR);
				initTable();
			}
		...
		btnConsultar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showProdutoFrame(Constantes.CONSULTAR);
			}
		});
	...
	private void initComponents() {
		...
		scrollPane.setViewportView(scrollPaneTableProduto);
		
		tableProduto = new JTable();
		tableProduto.setFont(new Font("Tahoma", Font.PLAIN, 9));
		scrollPaneTableProduto.setViewportView(tableProduto);
	...
	public static TableProdutoPanel getInstance() {
		if(Objects.isNull(TABLE_CLIENTE)) {
			TABLE_CLIENTE = new TableProdutoPanel();
		}
		return TABLE_CLIENTE;
	}
	
	private void listarProduto() {
		produtoeService = getProdutoService();
		page = produtoeService.listaPaginada(paginaAtual, tamanhoPagina);
	...
	private void initTable() {
		...
		model = new TableProdutoModel(page.getContent());
		...
		tableProduto.getTableHeader().setDefaultRenderer(renderHeader);
		
		RenderTable render = new RenderTable();
		
		for(int coluna = 0; coluna < model.getColumnCount(); coluna++) {
			tableProduto.setDefaultRenderer(model.getColumnClass(coluna), render);
		}
		
		TableColumn coluna = tableProduto.getColumnModel().getColumn(0);
		coluna.setMinWidth(50);
		coluna.setMaxWidth(60);
		coluna.setPreferredWidth(55);
		
		for(int col = 1; col<model.getColumnCount();col++ ) {
			coluna = tableProduto.getColumnModel().getColumn(col);
	...
	private void showProdutoFrame(int opcaoCadastro) {
		ProdutoView view = new ProdutoView(produtoe, opcaoCadastro);
	...
	private void getLinhaTabela() {
		...
		if(tableProduto.getSelectedRow() != -1) {
			linha = tableProduto.getSelectedRow();
			setColuna(tableProduto.getSelectedColumn());
			produtoe = model.getProduto(linha);
			linha = -1;
		}
		else {
	...
	public void setProdutoService(ProdutoService produtoeService) {
		this.produtoeService = produtoeService;
	}

	public void setProduto(Produto produtoe) {
