...
public class TableEnderecoPanel extends JPanel {
	private static final long serialVersionUID = -4694190107545197497L;
	JTable tableEndereco;
	JScrollPane scrollPane = new JScrollPane();
	JScrollPane scrollPaneTableEndereco = new JScrollPane();
	...
	private TableEnderecoModel model;
	private Page<Endereco> page;
	...
	private static TableEnderecoPanel TABLE_ENDERECO;

	public TableEnderecoPanel() {
		//setVisible(true);
		initComponents();
		eventHandler();
		initTable();
	...
	private void eventHandler() {
		...
		btnAdicionar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				showEnderecoFrame(Constantes.INCLUIR);
				initTable();
			}
		...
		btnAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showEnderecoFrame(Constantes.ALTERAR);
				initTable();
			}
		});
		btnRemover.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showEnderecoFrame(Constantes.EXCLUIR);
				initTable();
			}
		...
		btnConsultar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				getLinhaTabela();
				showEnderecoFrame(Constantes.CONSULTAR);
			}
		});
		...
		btnRelatorio.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				relatorio.setFile("relatorio_endereco");
	...
	private void initComponents() {
		...
		scrollPane.setViewportView(scrollPaneTableEndereco);
		
		tableEndereco = new JTable();
		tableEndereco.setFont(new Font("Tahoma", Font.PLAIN, 9));
		scrollPaneTableEndereco.setViewportView(tableEndereco);
	...
	public static TableEnderecoPanel getInstance() {
		if(Objects.isNull(TABLE_ENDERECO)) {
			TABLE_ENDERECO = new TableEnderecoPanel();
		}
		return TABLE_ENDERECO;
	}
	
	private void listarEndereco() {
		enderecoService = getEnderecoService();
		if(txtSearch.equals("")) {
			page = enderecoService.listaPaginada(paginaAtual, tamanhoPagina);
		}
		else {
			page = enderecoService.listaPaginada(paginaAtual, tamanhoPagina, txtSearch.getText());
		}
		if(paginaAtual == 1) {
	...
	private void initTable() {
		...
		model = new TableEnderecoModel(page.getContent());
		...
		tableEndereco.getTableHeader().setDefaultRenderer(renderHeader);
		
		RenderTable render = new RenderTable();
		
		for(int coluna = 0; coluna < model.getColumnCount(); coluna++) {
			tableEndereco.setDefaultRenderer(model.getColumnClass(coluna), render);
		}
		
		TableColumn coluna = tableEndereco.getColumnModel().getColumn(0);
		coluna.setMinWidth(50);
		coluna.setMaxWidth(60);
		coluna.setPreferredWidth(55);
		
		for(int col = 1; col<model.getColumnCount();col++ ) {
			coluna = tableEndereco.getColumnModel().getColumn(col);
	...
	private void showEnderecoFrame(int opcaoCadastro) {
		EnderecoView view = new EnderecoView(endereco, opcaoCadastro);
	...
	private void getLinhaTabela() {
		...
		if(tableEndereco.getSelectedRow() != -1) {
			linha = tableEndereco.getSelectedRow();
			setColuna(tableEndereco.getSelectedColumn());
			endereco = model.getEndereco(linha);
			linha = -1;
		}
		else {
